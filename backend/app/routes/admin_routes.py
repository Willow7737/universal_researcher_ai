...
@router.post("/users")
def create_user(data: UserCreate, db: Session = Depends(database.SessionLocal), _: models.User = Depends(deps.get_admin_user)):
...
@router.get("/users")
def list_users(db: Session = Depends(database.SessionLocal), _: models.User = Depends(deps.get_admin_user)):
